...
$.fn.calendar = function(options) {
	...
	this.each(function(i, _element) {
		...
		element.data('calendar', calendar); // TODO: look into memory leak implications
		calendar.render();
	});
...
var dateFormatters = {
	...
	w   : function(d, o) { // local
		return o.weekNumberCalculation;
	},
	W   : function(d) { // ISO
...
function AgendaEventRenderer() {
	...
	function slotEventStart(event) {
		if (event.start) {
			...
			return addMinutes(cloneDate(event.end), opt('defaultEventMinutes'));
		}
	}
...
fcViews.resourceShortDay = ResourceShortDayView;

function ResourceShortDayView(element, calendar) {
	var t = this;
	...
	// imports
	ResourceView.call(t, element, calendar, 'resourceShortDay');
	var opt = t.opt;
	var renderBasic = t.renderBasic;
	var formatDates = calendar.formatDates;
	var getResources = t.getResources;
...
setDefaults({
	...
	titleFormat: {
		...
		resourceShortDay: 'dddd, MMM d, yyyy'
	},
	columnFormat: {
		resourceShortDay: 'H:mm',
		resourceDay: 'H:mm',
		resourceMonth: 'M/d',
	...
	buttonText: {
		...
		resourceShortDay: 'resource short day',
...
function ResourceView(element, calendar, viewName) {
	...
	function buildHeadHTML() {
		...
		html +=
			...
			(showWeekNumbers && (viewName === 'resourceShortDay' || viewName === 'resourceDay' || viewName === 'resourceWeek') ? weekTitle : "&nbsp;") +
			"</th>";
		for (col=0; col<colCnt; col++) {
			date = indexDate(col);
			html +=
                ...
                ((viewName === 'resourceWeek' || viewName === 'resourceMonth') ?
                    htmlEscape(formatDate(date, colFormat)) + "</th>" :
                    buildHeaderTime(date, colFormat) + "</th>");
		}
    ...
    // new formated time header
    function buildHeaderTime(date, colFormat) {
		var html = '';
        var minutes = formatDate(date, 'mm');
        var hours = formatDate(date, 'H');
//console.log(colFormat);
        
        html += "<div class='timeline-time'>";
        if (minutes == '00') {
            html += "<div class='timeline-time-hours'>" + hours + "</div>";
        } else
            html += "<div class='timeline-time-minutes'>" + minutes + "</div>";
        
        html += "<div class='timeline-arrow-down'></div>";
        html += "</div>";
	...
	function buildBodyHTML() {
		...
		for (row=0; row<rowCnt; row++) {
            ...
            resourceExtra = resources[row]['extra'];

			html += "<tr class='fc-resourcerow-" + id + "' data-resourceid='" + id + "'>" +
			"<td class='fc-resourceName'>" +
        "<span>" + resourceName + "</span><br>" +
        "<span class='fc-resoureExtra'>" + resourceExtra + "</span>" +
      "</td>";
	...
	function setWidth(width) {
		...
		snapRatio = 1;
		if(viewName === 'resourceShortDay' || viewName === 'resourceDay') {
			snapRatio = opt('slotMinutes') / snapMinutes;
		}
	...
	function renderDayOverlay(overlayStart, overlayEnd, refreshCoordinateGrid, overlayRow) { // overlayEnd is exclusive
		...
		if (viewName == 'resourceShortDay' || viewName == 'resourceDay') {
            // must be done nicer
            if (viewName == 'resourceShortDay') {
                overlayEnd = Math.min(overlayEnd, addMinutes(cloneDate(rowStart), (parseTime(opt('maxTime'))-parseTime(opt('minTime')))));
            }
	...
	function datePositionLeft(eventDate) {
		var positionFunc = (viewName == 'resourceShortDay' || viewName === 'resourceDay') ? _timePositionLeft : _dayPositionLeft;
	...
	function timeOfDayCol(datetime) {
		...
		for ( var i = 0; i < colCnt; i++) {
			if (indexDate(i).getHours() == hours && indexDate(i).getMinutes() == minutes) {
                if (t.visEnd.getTime() === datetime.getTime()) {
                    return colCnt;
                } else {
                    return i;
                }
		...
		if (ev.which == 1 && opt('selectable')) { // which==1 means left mouse button
			...
			hoverListener.start(function(cell, origCell) { // TODO: maybe put cellDate/cellIsAllDay info in cell
				...
				if (cell && cellIsAllDay(cell) && resourceRO !== true) {
					...
					renderSelection(dates[0], dates[1], (viewName == 'resourceShortDay' || viewName == 'resourceDay' ? false : true), cell.row);
					row = cell.row;
				}else{
					dates = null;
				}
			...
			$(document).one('mouseup', function(ev) {
				...
				if (dates) {
					if (+dates[0] == +dates[1]) {
						reportDayClick(dates[0],(viewName == 'resourceShortDay' || viewName == 'resourceDay' ? false : true), ev, resources[row]);
					}
					reportSelection(dates[0], (viewName == 'resourceShortDay' || viewName == 'resourceDay' ? addMinutes(dates[1], opt('slotMinutes')) : dates[1]), (viewName == 'resourceShortDay' || viewName == 'resourceDay' ? false : true), ev, resources[row]);
				}
			});
		}
	}
...
function ResourceEventRenderer() {
	...
	t.resizableResourceEventStart = resizableResourceEventStart;
	t.resizableResourceEventEnd = resizableResourceEventEnd;
...
//console.log(seg.startCol);
//console.log(seg.endCol);
...
//console.log(rightCol);
			...
			html +=
				...
				">";
			html +=
				"<span class='fc-event-title' " + (skinCss ? " style='" + skinCss + "'" : "") + ">" +
                    htmlEscape(event.title ? event.title + " / "+event.product : event.product) +
            " B#" + event.book_nr + " P#" + event.project_nr +
            "</span><br>";
			if (!event.allDay && (seg.isStart || seg.iseEnd)) {
				html +=
			...
			html +=
				"<span class='fc-event-title' " + (skinCss ? " style='" + skinCss + "'" : "") + ">" +
                    htmlEscape(event.account) + "</span><br>" +
				"<span class='fc-event-title' " + (skinCss ? " style='" + skinCss + "'" : "") + ">" +
                    htmlEscape(event.description) + "</span><br></div>";
            if (seg.isStart && isEventResizable(event)) {
				html +=
					"<div class='ui-resizable-handle ui-resizable-" + (rtl ? 'e' : 'w') + "'>" +
	...
	function bindDaySeg(event, eventElement, seg) {
		...
		if (seg.isStart && isEventResizable(event) && isResourceEditable(event.resource)) {
			resizableResourceEventStart(event, eventElement, seg);
		}
		if (seg.isEnd && isEventResizable(event) && isResourceEditable(event.resource)) {
			resizableResourceEventEnd(event, eventElement, seg);
		}
		eventElementHandlers(event, eventElement);
	}
	...
	function draggableResourceEvent(event, eventElement) {
		...
		eventElement.draggable({
			...
			stop: function(ev, ui) {
				...
				if ((viewName == 'resourceShortDay' || viewName == 'resourceDay') && (minuteDelta || resourceDelta)) {
	...
	function resizableResourceEventStart(event, element, seg) {
		var rtl = opt('isRTL');
		var direction = rtl ? 's' : 'w';
		...
		element
			...
			.click(function(ev) {
				if (isResizing) {
					...
					ev.stopImmediatePropagation(); // prevent calendar eventClick handler from being called
		...
		handle.mousedown(function(ev) {
			...
			hoverListener.start(function(cell, origCell) {
				if (cell) {
					...
					if (viewName == 'resourceShortDay' || viewName == 'resourceDay') {
                        if (c > dateCell(event.end).col && seg.isEnd == true) {
                            return;
                        }
						minuteDelta = (opt('slotMinutes') * c*dis+dit) - (opt('slotMinutes') * origCell.col*dis+dit);
						visStart = event.start || addMinutes(cloneDate(event.end), opt('slotMinutes'));
						var newStart = addMinutes(cloneDate(visStart), minuteDelta);
					}
					else {
                        if (c > dateCell(event.end).col && seg.isEnd == true) {
                            return;
                        }
						dayDelta = dayDeltaStart = (7 + c*dis+dit) - (7 + origCell.col*dis+dit);
						visStart = event.start || cloneDate(event.end);
						// If weekends is set to false, add or remove days from dayDelta
						if (!opt('weekends') && (dayDelta > 0 || dayDelta < 0)) {
							if (dayDelta > 0) {
								for(var i=1; i<=dayDeltaStart; i++) {
						...
						newStart = addDays(eventStart(event), dayDeltaStart, true);
					}
					
					if (dayDelta || minuteDelta) {
						eventCopy.start = newStart;
					...
					if (viewName == 'resourceShortDay' || viewName == 'resourceDay') {
						renderDayOverlay(cloneDate(newStart), event.end, 1, origCell.row); // coordinate grid already rebuild at hoverListener.end
					}
					else {
						renderDayOverlay(addDays(cloneDate(newStart), 1), event.end, 1, origCell.row); // coordinate grid already rebuild at hoverListener.end
					}
				}
			}, ev);
			...
			function mouseup(ev) {
				...
				if (dayDelta) {
					if (!opt('weekends')) {
						// We have to add or remove days from event.start. Is there a better way?
						visStart = event.start || cloneDate(event.end);
						if (dayDelta > 0) {
							var daysToAdd = 0;
							...
							if (daysToAdd > 0) event.start = addDays(cloneDate(visStart), daysToAdd, true);
						}
						else {
							var daysToDel = 0;
							for(i=-1; i>=dayDelta+daysToDel; i--) {
							...
							if (daysToDel < 0) event.start = addDays(cloneDate(visStart), daysToDel, true);
						}
					}
				
					eventResize(this, event, dayDelta, 0, ev, null, true);
					// event redraw will clear helpers
				}
				else if (minuteDelta) {
					eventResize(this, event, 0, minuteDelta, ev, null, true);
				}
	...
	function resizableResourceEventEnd(event, element, seg) {
		var rtl = opt('isRTL');
		...
		handle.mousedown(function(ev) {
			...
			$('body')
				...
				if (cell) {
					...
					if (viewName == 'resourceShortDay' || viewName == 'resourceDay') {
                        if (c < dateCell(event.start).col && seg.isStart == true) {
                            return;
                        }
						minuteDelta = (opt('slotMinutes') * c*dis+dit) - (opt('slotMinutes') * origCell.col*dis+dit);
			...
			function mouseup(ev) {
				...
				if (dayDelta) {
					...
					eventResize(this, event, dayDelta, 0, ev, null, false);
					// event redraw will clear helpers
				}
				else if (minuteDelta) {
					eventResize(this, event, 0, minuteDelta, ev, null, false);
				}
				// otherwise, the drag handler already restored the old events
...
function View(element, calendar, viewName) {
	...
	// imports
	var defaultEventStart = t.defaultEventStart;
	...
	function eventStart(event) {
		return event.start ? cloneDate(event.start) : defaultEventStart(event);
	}
	...
	function eventResize(e, event, dayDelta, minuteDelta, ev, ui, start) {
		var eventId = event._id;
		elongateEvents(eventsByID[eventId], dayDelta, minuteDelta, start);
		trigger(
			'eventResize',
			e,
			event,
	...
	function elongateEvents(events, dayDelta, minuteDelta, start) {
		minuteDelta = minuteDelta || 0;
		for (var e, len=events.length, i=0; i<len; i++) {
			e = events[i];
			if (start) {
        e.start = addMinutes(addDays(eventStart(e), dayDelta, true), minuteDelta);
      }
      else {
...
//console.log(segments);
	...
	function buildHTMLForSegment(segment) {
		...
		if (seg.isStart && isEventResizable(event)) {
			html +=
				"<div class='ui-resizable-handle ui-resizable-" + (isRTL ? 'e' : 'w') + "'>" +
				"&nbsp;&nbsp;&nbsp;" + // makes hit area a lot better for IE6/7
				"</div>";
		}
		if (seg.isEnd && isEventResizable(event)) {
	...
	function bindDaySeg(event, eventElement, segment) {
		...
		if (
			(segment.isStart || segment.isEnd) && // only allow resizing on the final segment for an event
